...
class _DeliverySetupState extends State<DeliverySetup> with TickerProviderStateMixin {
  ...
  Widget _buildTrackDeliveryTab() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('orders')
          .where('status', whereIn: ['ready', 'on_way', 'on_mid_way'])
          .orderBy('createdAt', descending: true)
          .snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      case 'ready':
        return Row(
          children: [
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: () => _updateDeliveryStatus(orderId, 'on_way'),
                icon: const Icon(Icons.local_shipping, size: 16),
                label: const Text('Dispatch'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.green,
  ...
  Future<void> _markAsDispatched(String orderId) async {
    try {
      ...
      _showMessage('Order marked as dispatched!');
    } catch (e) {
      _showMessage('Error marking as dispatched: $e');
    }
  }
