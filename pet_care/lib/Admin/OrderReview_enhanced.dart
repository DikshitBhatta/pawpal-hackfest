...
class OrderReviewEnhanced extends StatefulWidget {
  const OrderReviewEnhanced({Key? key}) : super(key: key);

  @override
  _OrderReviewEnhancedState createState() => _OrderReviewEnhancedState();
}

class _OrderReviewEnhancedState extends State<OrderReviewEnhanced> with TickerProviderStateMixin {
  ...
  Future<String> _getAddressFromCoordinates(double lat, double lng) async {
    String key = '${lat.toStringAsFixed(6)},${lng.toStringAsFixed(6)}';
    ...
    String fallback = "Lat: ${lat.toStringAsFixed(4)}, Lng: ${lng.toStringAsFixed(4)}";
  ...
  Future<void> _openDeliveryLocation(Map<String, dynamic> orderData) async {
    ...
    if (deliveryLatitude != null && deliveryLongitude != null && 
      ...
      // Get readable address and show in dialog
      String address = await _getAddressFromCoordinates(deliveryLatitude, deliveryLongitude);
      _showMessage(
        'Delivery Location:\n$address\n\nCoordinates: ${deliveryLatitude.toStringAsFixed(4)}, ${deliveryLongitude.toStringAsFixed(4)}',
        ContentType.help,
      );
      return;
    }
        ...
        if (userDoc.exists) {
          ...
          if (petsQuery.docs.isNotEmpty) {
            ...
            if (petLat != null && petLong != null) {
              // Get readable address for pet location
              String petAddress = await _getAddressFromCoordinates(petLat, petLong);
              ...
              await FirebaseFirestore.instance
                ...
                'deliveryAddress': petAddress,
                'deliveryCity': '${petData['city'] ?? ''}',
              });
              
              _showMessage(
                'Pet Location Found:\n$petAddress\n\nCoordinates: ${petLat.toStringAsFixed(4)}, ${petLong.toStringAsFixed(4)}\n(Saved for future deliveries)',
                ContentType.success,
              );
              return;
            }
