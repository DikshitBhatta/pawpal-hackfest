...
class GeminiMealService {
  ...
  static Future<Map<String, dynamic>?> generateOptimalPersonalizedMeal({
    ...
    try {
      ...
      if (petData == null) {
        return {
          'error': 'Could not fetch pet data. Please ensure the pet exists.'
        };
      }
      ...
      return {
        'error': 'Error generating personalized meal plan. Please try again.'
      };
    }
  }
  ...
  static Future<Map<String, dynamic>?> generateOptimalPersonalizedMealWithData({
    ...
    try {
      ...
      final availableIngredients =
          await IngredientService.fetchAvailableIngredients();
      if (availableIngredients.isEmpty) {
        return {
          'error':
              'No ingredients available in inventory. Please contact administrator.'
        };
      }

      // Filter ingredients based on pet allergies
      final safeIngredients =
          _filterIngredientsForAllergies(availableIngredients, petData);

      // Filter supplements and special diet items
      final availableSupplements = safeIngredients
          .where((item) =>
      ...
      final prompt = _buildOptimalMealPrompt(
          petData, safeIngredients, availableSupplements);
  ...
  static double calculateMealPrice(
      Map<String, dynamic> mealPlan, List<Map<String, dynamic>> inventory) {
      ...
      if (mealPlan['ingredients'] != null) {
        ...
        for (var ingredient in ingredients) {
          ...
          if (inventoryItem.isNotEmpty) {
            double pricePerGram =
                (inventoryItem['pricePerUnit'] ?? 0.0).toDouble() /
                    1000; // Convert price per kg to per gram
      ...
      if (mealPlan['supplements_vitamins_minerals'] != null) {
        ...
        for (var supplementName in supplements) {
          var inventoryItem = inventory.firstWhere(
            (item) =>
                item['name']?.toString().toLowerCase() ==
                supplementName.toString().toLowerCase(),
            orElse: () => {},
          );
      ...
      if (mealPlan['snacks_treats_special_diet'] != null) {
        ...
        for (var snackName in snacks) {
          var inventoryItem = inventory.firstWhere(
            (item) =>
                item['name']?.toString().toLowerCase() ==
                snackName.toString().toLowerCase(),
            orElse: () => {},
          );
    ...
    try {
      ...
      if (response.text != null) {
        ...
        try {
          ...
          // Calculate meal price
          double totalPrice =
              calculateMealPrice(editedMealPlan, safeIngredients);
          ...
          try {
            String jsonResponse = _extractJsonFromResponse(response.text!);
            jsonResponse = _aggressiveCleanJson(jsonResponse);
            final Map<String, dynamic> editedMealPlan =
            ...
            return {
              'error':
                  'Failed to parse meal plan. Please try again with a simpler request.'
            };
          }
        }
      }
  ...
  static String _buildEditMealPrompt(
    ...
    double weight =
        double.tryParse(petData['weight']?.toString() ?? '0') ?? 0.0;
    ...
    List<dynamic> currentSupplements =
        currentMealPlan['supplements_vitamins_minerals'] ?? [];
    List<dynamic> currentSnacks =
        currentMealPlan['snacks_treats_special_diet'] ?? [];
    String currentPreparation =
        currentMealPlan['preparation_instructions'] ?? '';
    ...
    if (petData['customAllergies'] != null &&
        petData['customAllergies'].toString().isNotEmpty) {
      allergies.addAll(petData['customAllergies']
          .toString()
          .split(',')
          .map((e) => e.trim()));
    }
    ...
    prompt.writeln(
        'You are a professional canine nutritionist AI with veterinary expertise.');
    prompt.writeln('');
    prompt.writeln(
        'Edit the existing meal plan based on the user\'s specific request while maintaining nutritional balance for the dog.');
    prompt.writeln('');
    ...
    if (allergies.isNotEmpty) {
      prompt.writeln(
          '• Allergies: ${allergies.join(', ')} (⚠️ NEVER include these ingredients)');
    }
    ...
    for (int i = 0; i < availableIngredients.length; i++) {
      ...
      prompt
          .writeln('    "price_per_unit": ${ingredient['pricePerUnit'] ?? 0},');
      prompt.writeln(
          '    "stock_quantity": ${ingredient['stockQuantity'] ?? 0},');
    ...
    prompt.writeln(
        'Return only the following JSON structure with the edited meal plan:');
    prompt.writeln('');
    prompt.writeln('```json');
    prompt.writeln('{');
    prompt.writeln(
        '  "meal_name": "Updated Meal Name (reflect the changes made)",');
    prompt.writeln(
        '  "recommendation_reason": "Brief explanation of why this edited meal is optimal.",');
    ...
    prompt.writeln('  ],');
    prompt.writeln(
        '  "medical_care_instructions": "Care instructions if health conditions exist, or null if none",');
    prompt.writeln('  "activity_recommendation": {');
    prompt.writeln(
        '    "recommendation": "Specific activity recommendation if needed",');
    prompt.writeln('    "reason": "Why this activity level is recommended",');
    prompt.writeln(
        '    "activities": ["Specific activity 1", "Specific activity 2"]');
    prompt.writeln('  },');
    prompt.writeln(
        '  "preparation_instructions": "Updated preparation instructions reflecting the changes"');
    prompt.writeln('}');
    prompt.writeln('```');
    prompt.writeln('');
    prompt.writeln('🛑 IMPORTANT REQUIREMENTS:');
    prompt.writeln(
        '- Apply the user\'s edit request accurately while maintaining nutritional balance');
    prompt.writeln(
        '- Only use ingredients from the provided inventory - NO EXCEPTIONS');
    prompt.writeln(
        '- Provide a short, one-line recommendation reason explaining why this edited meal is optimal');
    prompt.writeln(
        '  Examples: "Updated for better protein balance.", "Enhanced with additional omega-3 for joint support.", "Modified for weight management goals."');
    prompt.writeln('- Respect all pet allergies and dietary restrictions');
    prompt.writeln(
        '- Keep portion sizes appropriate for the dog\'s weight and activity level');
    prompt.writeln('- Update the meal name to reflect the changes made');
    prompt
        .writeln('- Update preparation instructions if cooking methods change');
  ...
  static String _buildOptimalMealPrompt(
    ...
    int dailyActivityMinutes =
        int.tryParse(petData['dailyActivityMinutes']?.toString() ?? '0') ?? 0;
    ...
    String existingHealthCondition =
        petData['ExistingHealthCondition']?.toString() ?? '';
    ...
    if (petData['healthGoalPriorities'] != null) {
      healthGoalPriorities =
          Map<String, int>.from(petData['healthGoalPriorities']);
    }
    String customHealthGoal = petData['customHealthGoal']?.toString() ?? '';
    int customHealthGoalPriority =
        petData['customHealthGoalPriority']?.toInt() ?? 0;
    ...
    prompt.writeln('');
    prompt.writeln(
        'Generate the **best possible personalized meal** for this dog based on their comprehensive profile, using only ingredients from the provided inventory.');
    prompt.writeln(
        'Consider ALL aspects of the dog\'s health, physical condition, activity level, digestive health, and dietary needs.');
    prompt.writeln(
        'Along with food suggest nutritional supplements, vitamins and minerals from the inventory.');
    prompt.writeln('');
    prompt.writeln(
        '**IMPORTANT**: Instead of using snacks from inventory, AI should suggest appropriate homemade/natural snacks based on the dog\'s health profile.');
    prompt.writeln('');
    prompt.writeln('The meal should:');
    prompt.writeln(
        '- Be optimized for the dog\'s breed characteristics, age, weight, gender, and neutered status');
    prompt.writeln(
        '- Account for their current body condition (slim/normal/overweight) and activity level');
    prompt.writeln(
        '- Consider their digestive health based on stool analysis (color, frequency, consistency, quantity)');
    prompt.writeln(
        '- Align with their current feeding frequency and daily activity minutes');
    prompt.writeln(
        '- Address any existing health conditions and medication interactions');
    prompt.writeln(
        '- Incorporate their favorite foods while avoiding all allergens');
    prompt.writeln(
        '- Include a **creative meal name** that reflects the dog\'s profile');
    prompt.writeln(
        '- Use **only ingredients** and **supplements** from the provided list');
    prompt.writeln(
        '- Include exact **ingredient quantities (in grams)** appropriate for their weight and condition');
    prompt.writeln(
        '- Provide **step-by-step preparation instructions** (for the kitchen)');
    prompt.writeln(
        '- Recommend **nutritional supplements, vitamins and minerals from the list** that suit the dog\'s health profile');
    prompt.writeln(
        '- Generate **AI-suggested natural snacks** (not from inventory) that complement the meal');
    prompt.writeln(
        '- Include **medical care instructions** if the dog has any health conditions or recent medical procedures');
    prompt.writeln(
        '- Provide **activity recommendations** based on current activity level, weight, and health status');
    prompt.writeln('');
    prompt.writeln(
        '💡 DO NOT suggest any ingredient or supplement not included in the inventory list.');
    prompt.writeln(
        '💡 Pay special attention to portion control based on body condition and activity level.');
    prompt.writeln(
        '💡 Consider digestive health indicators when selecting ingredients and preparation methods.');
    prompt.writeln(
        '💡 For snacks, suggest healthy homemade options that complement the meal (not from inventory).');
    prompt.writeln(
        '💡 If medical conditions exist, provide simple care instructions alongside the meal plan.');
    prompt.writeln(
        '💡 Evaluate current activity level and provide recommendations if adjustments are needed.');
    prompt.writeln('');
    prompt.writeln('🐶 DOG PROFILE:');
    prompt.writeln('• Pet Name: $petName');
    prompt.writeln('• Pet Type: $petType');
    ...
    if (healthGoals.isNotEmpty) {
      ...
      prompt.writeln(
          '• Health Goals (in priority order): ${prioritizedGoals.join(', ')}');

      // Add priority explanation for AI
      if (healthGoalPriorities.isNotEmpty) {
        prompt.writeln(
            '  📍 IMPORTANT: Focus meal recommendations primarily on Priority 1 goals, then Priority 2, etc.');
      }
    }
    ...
    if (customHealthGoal.isNotEmpty) {
      if (customHealthGoalPriority > 0) {
        prompt.writeln(
            '• Custom Health Goal: $customHealthGoal (Priority: $customHealthGoalPriority)');
      } else {
    ...
    for (int i = 0; i < availableIngredients.length; i++) {
      ...
      prompt.writeln('    "name": "${ingredient['name']}",');
      prompt
          .writeln('    "category": "${ingredient['category'] ?? 'Unknown'}",');
      prompt.writeln(
          '    "price_per_gram": ${((ingredient['pricePerUnit'] ?? 0.0) / 1000).toStringAsFixed(4)},');
      prompt.writeln(
          '    "stock_available": "${ingredient['stock'] ?? 0} ${ingredient['unit'] ?? 'units'}",');

      // Add nutrition info if available
      Map<String, dynamic> nutrition = {};
      if (ingredient['protein'] != null)
        nutrition['protein'] = ingredient['protein'];
      if (ingredient['fat'] != null) nutrition['fat'] = ingredient['fat'];
      if (ingredient['fiber'] != null) nutrition['fiber'] = ingredient['fiber'];
      if (ingredient['calcium'] != null)
        nutrition['calcium'] = ingredient['calcium'];
      if (ingredient['vitaminA'] != null)
        nutrition['vitaminA'] = ingredient['vitaminA'];
      if (ingredient['omega3'] != null)
        nutrition['omega3'] = ingredient['omega3'];
    ...
    prompt.writeln(
        '  "recommendation_reason": "Tailored for joint health and weight control.",');
    prompt.writeln('  "ingredients": [');
    prompt.writeln('    {');
    ...
    prompt.writeln('  ],');
    prompt.writeln(
        '  "medical_care_instructions": "Keep incision area clean and dry. Monitor for swelling.",');
    prompt.writeln('  "activity_recommendation": {');
    prompt
        .writeln('    "recommendation": "Increase daily walks to 45 minutes",');
    prompt.writeln(
        '    "reason": "Current activity level too low for weight management",');
    prompt.writeln(
        '    "activities": ["Morning walk: 20 minutes", "Evening walk: 25 minutes"]');
    prompt.writeln('  },');
    prompt.writeln(
        '  "preparation_instructions": "Boil the chicken for 15 minutes. Cook brown rice separately. Mix together and top with Fish Oil. Serve warm. Ensure water is available."');
    prompt.writeln('}');
    prompt.writeln('```');
    prompt.writeln('');
    ...
    prompt.writeln(
        '- Only use the provided inventory for ingredients and supplements - NO EXCEPTIONS');
    prompt.writeln('- Include cooking method suitable for dogs');
    prompt.writeln(
        '- Provide a short, one-line recommendation reason explaining why this meal is optimal for this specific dog');
    prompt.writeln(
        '  Examples: "Tailored for joint health and weight control.", "Optimized for senior digestive support.", "High-energy formula for active working dogs.", "Gentle on sensitive stomach with digestive support."');
    prompt.writeln(
        '- For snacks, suggest healthy homemade/natural options that complement the meal (NOT from inventory)');
    prompt.writeln(
        '  Examples: "Frozen carrot sticks", "Dehydrated sweet potato chips", "Ice cubes with low-sodium broth", "Apple slices (no seeds)"');
    prompt.writeln(
        '- If medical conditions exist, provide simple care instructions alongside the meal');
    prompt.writeln(
        '  Examples: "Keep incision area clean and dry", "Monitor for swelling", "Ensure adequate rest periods"');
    prompt.writeln(
        '- Evaluate current activity level and provide recommendations if needed');
    prompt.writeln(
        '  Include specific activities and duration based on dog\'s weight, age, and health status');
    prompt
        .writeln('- Recommend only needed supplements based on health profile');
    prompt.writeln(
        '- Consider the dog\'s specific health profile, favorites, and allergies');
    prompt.writeln(
        '- Optimize portion sizes for the dog\'s weight, body condition, and activity level');
    prompt.writeln(
        '- Account for gender and neutered status in nutritional needs');
    prompt.writeln(
        '- Consider digestive health indicators (stool analysis) for ingredient selection');
    prompt.writeln('- Align meal timing with current feeding frequency');
    prompt
        .writeln('- Factor in daily activity minutes for caloric requirements');
    prompt
        .writeln('- Address any existing health conditions through nutrition');
    prompt.writeln(
        '- Ensure meal supports optimal body condition (weight management if needed)');

    return prompt.toString();
  }

  /// Fetch comprehensive pet data from Firestore
  static Future<Map<String, dynamic>?> _fetchPetDataFromFirestore(
      String userEmail, String petId) async {
    try {
  ...
  static List<Map<String, dynamic>> _filterIngredientsForAllergies(
      List<Map<String, dynamic>> ingredients, Map<String, dynamic> petData) {
  ...
  static Map<String, dynamic> _generateActivityPersonalizationNote(
      Map<String, dynamic> petData) {
    ...
    if (isOverweight) {
      personalizationNote =
          "Based on $petName's weight ($weight$weightUnit) and body condition, this meal plan focuses on weight management with lean proteins and controlled portions.";

      if (currentActivity.toLowerCase() == 'low') {
        activityRecommendation =
            "Gradually increase activity to 30-45 minutes daily with gentle walks and play sessions to support healthy weight loss.";
        recommendationType = 'increase';
      } else if (currentActivity.toLowerCase() == 'medium') {
        activityRecommendation =
            "Increase activity to 45-60 minutes daily with more vigorous exercise like longer walks, swimming, or interactive play.";
        recommendationType = 'increase';
      } else {
        activityRecommendation =
            "Maintain current high activity level while monitoring caloric intake to achieve gradual weight loss.";
        recommendationType = 'maintain';
      }
    } else if (isUnderweight) {
      personalizationNote =
          "Based on $petName's lean body condition, this meal plan includes calorie-dense ingredients to support healthy weight gain.";

      if (currentActivity.toLowerCase() == 'high') {
        activityRecommendation =
            "Consider reducing intense exercise to 30-40 minutes daily to help with weight gain while maintaining muscle tone.";
        recommendationType = 'decrease';
      } else {
        activityRecommendation =
            "Maintain moderate activity (20-30 minutes daily) to build muscle while supporting weight gain.";
        recommendationType = 'maintain';
      }
    } else if (hasHealthIssues) {
      personalizationNote =
          "Based on $petName's health conditions, this meal plan includes easily digestible ingredients and therapeutic nutrients.";

      if (currentActivity.toLowerCase() == 'high') {
        activityRecommendation =
            "Reduce to gentle, low-impact activities (15-25 minutes daily) until health improves. Focus on mental stimulation.";
        recommendationType = 'decrease';
      } else {
        activityRecommendation =
            "Continue with gentle activities and gradually increase based on health improvement and vet guidance.";
        recommendationType = 'maintain';
      }
    } else {
      // Healthy weight and condition
      personalizationNote =
          "Based on $petName's optimal weight ($weight$weightUnit), $currentActivity activity level, and healthy condition, this meal plan maintains their excellent health.";

      if (currentActivity.toLowerCase() == 'low' && dailyActivityMinutes < 20) {
        activityRecommendation =
            "Consider increasing activity to 25-35 minutes daily for better cardiovascular health and mental stimulation.";
        recommendationType = 'increase';
      } else if (currentActivity.toLowerCase() == 'high' &&
          dailyActivityMinutes > 90) {
        activityRecommendation =
            "Excellent activity level! Maintain 60-90 minutes of varied exercise to keep $petName healthy and happy.";
        recommendationType = 'maintain';
      } else {
        activityRecommendation =
            "Current activity level is perfect! Continue with $currentActivity intensity exercise to maintain optimal health.";
        recommendationType = 'maintain';
      }
    }
    ...
    return {
      ...
      'recommended_daily_minutes': _getRecommendedMinutes(
          recommendationType, currentActivity, dailyActivityMinutes),
    };
  }

  /// Get recommended daily activity minutes based on recommendation type
  static int _getRecommendedMinutes(
      String recommendationType, String currentActivity, int currentMinutes) {
  ...
  static String _buildPersonalizedMealPrompt(Map<String, dynamic> petData,
      List<Map<String, dynamic>> availableIngredients) {
    final prompt = StringBuffer();
    ...
    if (healthGoals.isNotEmpty) {
      ...
      if (healthGoalPriorities.isNotEmpty) {
        prompt.writeln(
            '  📍 CRITICAL: Prioritize meal ingredients and recommendations based on Priority 1 goals first, then Priority 2, etc.');
      }
    }
    ...
    if (favoritefoods.isNotEmpty) {
      prompt.writeln(
          '• Favorite Foods: ${favoritefoods.join(', ')} (prefer these ingredients when possible)');
    }
    ...
    if (allergies.isNotEmpty) {
      prompt.writeln(
          '• Food Allergies: ${allergies.join(', ')} (AVOID these ingredients)');
    }
    ...
    if (poopDescription.isNotEmpty && poopDescription != 'Normal') {
      prompt.writeln(
          '• Current Digestive Status: $poopDescription (consider digestive health)');
    }
    ...
    if (medicalFile.isNotEmpty) {
      prompt.writeln(
          '• Medical Records: Available (consider any health conditions)');
    }
    ...
    for (var ingredient in availableIngredients) {
      String description =
          IngredientService.getIngredientDescription(ingredient);
    ...
    prompt.writeln('📋 REQUIREMENTS:');
    prompt.writeln(
        '1. Create ONE optimal meal plan specifically tailored to this pet');
    prompt.writeln('2. Use ONLY ingredients from the available list above');
    prompt.writeln(
        '3. Consider ALL pet health factors: breed, weight, age, activity, health goals, favorites, allergies, digestive status');
    prompt.writeln(
        '4. ⭐ PRIORITY SYSTEM: When health goals have priorities, focus primarily on Priority 1 goals in ingredient selection and meal composition, then incorporate Priority 2, etc.');
    prompt.writeln('5. Provide specific portions based on pet size and needs');
    prompt.writeln(
        '6. Include preparation instructions suitable for pet consumption');
    prompt.writeln(
        '7. Explain health benefits specific to this pet\'s profile, emphasizing how the meal addresses highest priority health goals');
    prompt.writeln('8. Consider digestive health and adjust accordingly');
    prompt.writeln(
        '9. Incorporate favorite foods when nutritionally appropriate and aligned with health goal priorities');
    prompt.writeln('');
    prompt.writeln('PROVIDE:');
    prompt.writeln(
        '• Creative meal name reflecting the pet\'s personality or needs');
    prompt.writeln('• Detailed nutritional breakdown');
    prompt.writeln(
        '• Three meals (breakfast, lunch, dinner) with specific ingredients and portions');
    prompt.writeln('• Preparation instructions for each meal');
    prompt.writeln(
        '• Health benefits explanation for this specific pet, clearly explaining how the meal addresses Priority 1 health goals first');
    prompt.writeln('• Feeding guidelines and schedule');
    prompt.writeln(
        '• Any special considerations based on the pet\'s profile and priority health goals');
  ...
  static String _buildMealPlanPrompt({
    ...
    if (dietaryRestrictions.isNotEmpty) {
      prompt
          .writeln('• Dietary Restrictions: ${dietaryRestrictions.join(', ')}');
    }
    ...
    prompt.writeln(
        '3. Use structured percentage format for nutrition (e.g., "Protein: 25-30%")');
    prompt.writeln('4. Include breakfast, lunch, and dinner for each option');
    prompt.writeln(
        '5. Make each meal plan genuinely different, not variations of the same plan');
