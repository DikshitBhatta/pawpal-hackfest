  ...
  login(email,password) async{
    ...
    try{
      await FirebaseAuth.instance.signInWithEmailAndPassword(email: email, password: password).then((value) async{
        ...
        String userEmail = EmailController.value.text.trim();
        if (userEmail.isEmpty) {
          uiHelper.customAlertBox((){},context, "Email is required");
          return;
        }
        ...
        if (userRole == "admin") {
          ...
          // Navigate to User Dashboard (HomeScreen)
          await Navigator.pushReplacement(
            context, 
            MaterialPageRoute(builder: (context) => petScreenDynamicDark(userData: userData))
          );
        }
      });
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Opacity(opacity: 0.5,
                        child: Image(height: MediaQuery.of(context).size.height * 0.25,image: AssetImage("assets/images/petPic.png"))),
                    SizedBox(height: 30,),
                Padding(
